@php
    $user = auth()->user();
    $userCourses = $user->orders()->with('orderItems.course')->get()->pluck('orderItems')->flatten();
    $userWishlist = $user->wishlist()->with('course')->get();
@endphp
...
<ul class="generic-list-item off-canvas-menu-list off--canvas-menu-list pt-35px">
    ...
    <li>
        <a href="{{ route('user.dashboard') }}">
            ...
            </svg> My Courses ({{ $userCourses->count() }})
        </a>
        @if($userCourses->count() > 0)
            <ul class="sub-menu">
                @foreach($userCourses as $courseItem)
                    <li>
                        <a href="{{ route('course-details', $courseItem->course->course_name_slug) }}">
    ...
    <a href="{{ route('user.wishlist.index') }}">
        ...
        @if($wishlistCount > 0)
            <span class="badge bg-danger ml-1">{{ $wishlistCount }}</span>
        @endif
    </a>
</li>
